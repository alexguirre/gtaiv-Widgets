...
// Based on ExampleAppLog from imgui_demo.cpp
struct Log
{
	ImGuiTextBuffer Buf;
...
static Log gPrintLog{};     // for PRINT* natives
static Log gDebugFileLog{}; // for SAVE_*_TO_DEBUG_FILE natives
...
void LogWindow::Draw()
{
	...
	if (ImGui::Begin("Logs", &Open))
	{
		if (ImGui::BeginTabBar("##Tabs"))
		{
			...
			if (ImGui::BeginTabItem("Debug File"))
			{
				gDebugFileLog.Draw();
				ImGui::EndTabItem();
			}
...
void LogWindow::AddPrintLog(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	gPrintLog.AddLogV(fmt, args);
	va_end(args);
}
void LogWindow::AddDebugFileLog(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	gDebugFileLog.AddLogV(fmt, args);
	va_end(args);
}
...
void Log::AddLogV(const char* fmt, va_list args)
{
...
void Log::Draw()
{
	...
	else
	{
		...
		// A) random access into your data, and B) items all being the  same height, both of
		// which we can handle since we an array pointing to the beginning of each line of
		// text. When using the filter (in the block of code above) we don't have random
		// access into the data to display anymore, which is why we don't use the clipper.
		// Storing or skimming through the search result would make it possible (and would
		// be recommended if you want to search through tens of thousands of entries)
